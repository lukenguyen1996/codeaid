<!--
TeamBasic
Wesley Murray, Ryland Atkins, Luke Nguyen, Dominic Peluso

This is the file used to test the students.
...
$_SESSION['objectives'] = $_POST['objectives'];
	...
	echo '<form style="float:left;" action="student_objective_material.php" method="post">';
	echo '<input type="hidden" name="objective_id" value=$objectiveid><input type="submit" value="Help">';
	echo '<input type="hidden" name="quiz_objectives" value='.$_POST['quiz_objectives'].'></form>';
	//check if admin
...
foreach($objectives as $objective_id) {
	...
	$question_ids = get("question,question_id","questions NATURAL JOIN objectives","objective_id=$objective_id AND question_id NOT IN (SELECT questions.question_id FROM questions NATURAL JOIN student_attempts WHERE student_id=$student_id AND solution LIKE answer)","");
